...
class TestCallToolWalletEarnGate(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bypass_cap_blocked_when_disabled(self):
        """Phase 6.1: tool_loop 固定 bypass_cap=False，门控关闭时拒绝。
        零花钱/打赏改走 API_SECRET 保护的后端 API，不通过 tool_loop。"""
        call_made = {"v": False}
        ...
        with patch("tool_loop._money_earning_enabled", return_value=False), \
             patch.dict(tool_loop.TOOL_REGISTRY, {
                 "wallet_earn": {
                     ...
                     "fixed_args": {"wallet_id": "test", "meta": {}, "bypass_cap": False},
                 }
             }):
            res = await tool_loop.call_tool("wallet_earn",
                                    {"amount": 25, "source_key": "allowance_2026W33",
                                     "reason": "零花钱"})
        self.assertFalse(res["ok"])
        self.assertFalse(call_made["v"])

    async def test_bypass_cap_false_allowed_when_enabled(self):
        """money_earning_enabled=true → 不被门控拦截。"""
        call_made = {"v": False}
...
class TestServerWalletEarnGate(unittest.IsolatedAsyncioTestCase):
    async def test_mcp_wallet_earn_rejected_when_disabled(self):
        """Phase 6.1: server.wallet_earn（无bypass_cap）+ 关闭 → 返回 MONEY_EARNING_DISABLED。"""
        import server
    ...
    async def test_mcp_bypass_cap_true_blocked_when_disabled(self):
        """Phase 6.1: server.wallet_earn 移除了 bypass_cap 参数，门控关闭时拒绝。"""
        import server
        ...
        with patch("gateway._money_earning_enabled", return_value=False), \
            ...
            ret = await server.wallet_earn(amount=25, source_key="allowance_2026W33",
                                           reason="零花钱")
        # 门控关闭时返回 dict（非字符串），含 error_code
        self.assertIsInstance(ret, dict)
        self.assertFalse(ret.get("ok", False))
        m_hs.assert_not_called()

    async def test_mcp_bypass_cap_false_allowed_when_enabled(self):
        """server.wallet_earn（无bypass_cap）+ 开启 → 不拦截。"""
        import server
        fake_ret = {"ok": True, "message": "入账成功", "data": {"balance": 110}}
        with patch("gateway._money_earning_enabled", return_value=True), \
            ...
            ret = await server.wallet_earn(amount=10, source_key="task_1", reason="test")
        self.assertIsInstance(ret, dict)
        self.assertTrue(ret.get("ok", False))
