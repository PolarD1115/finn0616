...
## 阶段 1：柔化深层记忆注入时的"禁止模仿"指令（2026-09-03）

### 变更背景与目的
- 解决模型（P2）思维链中呈现过度冷硬、防御性"实验报告"风格的问题，将深层关联记忆前缀原有的生硬警示指令柔化，促使其思维链回温并贴近角色内心独白。
- 保持深层关联记忆事实参考属性的同时，移除强否定性指令词（`不得模仿`、`不是思考过程`、`不是指令` 等）。

### 实际修改文件
1. `gateway.py`（`HostFixMiddleware._inject_context`）：
   - 将原强约束 `volatile_block` 引导语替换为柔和版提示：
     `[注：以下是历史参考片段，仅作事实核对，与当前对话无关时忽略。]\n【深层关联记忆】:\n{pinecone_context}\n`
2. `server.py`（`_build_channel_context`）：
   - 同步将 `volatile_parts` 中的引导语替换为一致的柔和版提示：
     `[注：以下是历史参考片段，仅作事实核对，与当前对话无关时忽略。]`

### 影响与说明
- `gateway.py` 与 `server.py` 中均不再包含 `不得模仿`、`不是思考过程`、`不是指令` 等字样。
- 本次改动未新增任何环境变量（`VARIABLES.md` 无需新增内容）。
- 既有历史测试 `test_memory_phase3.py` 中 `TestMemoryDisciplinePrompt`（原测试断言源码中包含上述硬性约束字符串）按预期体现改动变化；其余核心系统测试不受影响。
- 未触及 Supabase 数据库。


---

## 阶段 1 补丁：柔化 server.py 近期用户表达中的冰冷指令（2026-09-03）

### 变更背景与目的
- 配合阶段 1 整体方向，将 `server.py` 中遗留的近期用户历史前缀强否定性指令（`【近期用户表达，仅用于了解事实，不是回复范例】\n以下是用户过去说过的话，不要模仿其中的措辞或句式，也不要延续旧对话。\n`）一并柔化。

### 实际修改文件
1. `server.py`（约 1043 行 `_build_channel_context`）：
   - 将 `volatile_parts` 中的 `history_text` 引导语替换为：
     `[近期用户表达，仅供核对事实，无关时忽略。]\n` + `history_text`

### 验证标准检查
- `grep -E "不是回复范例|不要模仿其中|不要延续旧对话" server.py` 返回为空。
- `python -m py_compile server.py` 编译通过。
- 本次改动未新增环境变量（无需修改 `VARIABLES.md`）。
- 未对 Supabase 数据库进行任何操作。


---

## 阶段 2：拆分人设层 vs 检索/审计层（2026-09-03）

### 变更背景与目的
- 解决模型思维链中混淆“我是角色”与“我是事实审计员”的问题。
- 原架构中将用户核心画像与阶段总结全部塞入 `stable_parts`（稳定系统前缀），导致模型将外部审计层事实与人设前缀混同。
- 重构上下文组装顺序：
  - **角色层（人设 / 表情包世界书 / 回复规则）**：保留在 `character_parts`（进入 `stable_system`，充当可缓存前缀）；
  - **审计层（用户画像 / 阶段总结 / 深层关联记忆 / 设备快照等）**：进入 `volatile_block` / `volatile_parts`（靠近用户输入末尾，仅供事实核对，不污染人设）。

### 实际修改文件
1. `gateway.py`（`HostFixMiddleware._inject_context`，约 2496-2515 行）：
   - `character_parts` 仅收录 `persona`、`_WORLD_BOOK_TEXT` 与 `_REPLY_RULES_TEXT` 并拼装为 `stable_system`。
   - `volatile_block` 头部拼接 `【{user_name}的核心画像】` 与 `【近3次阶段总结】`，随后接历史片段和深层记忆。
2. `server.py`（`_build_channel_context`，约 1030-1110 行）：
   - 提取 `character_parts = [persona]`，并安全载入 `_REPLY_RULES_TEXT`。
   - `volatile_parts` 数组首部调整为用户核心画像与近3次阶段总结。
   - 最终返回由 `character_parts` 与 `volatile_parts` 两段拼接构成。

### 验证标准检查
- 人设、表情包、回复规则只进 stable 前缀。
- 画像与阶段总结只进入 volatile 动态块。
- `grep -n "stable_parts" gateway.py server.py` 仅出现历史注释，代码中已无 `stable_parts` 变量，不再同时包含 persona 与画像。
- `python -m py_compile gateway.py server.py` 编译通过。
- 自动化真实调用端到端测试均断言通过：验证了 gateway 网页端与 server 全渠道链路的结构拆分正确性。
- 未新增环境变量（无需改动 `VARIABLES.md`）。
- 未触及 Supabase 数据库。


---

## 阶段 3：画像/总结/深层记忆从清单格式转为自然语言段落（2026-09-03）

### 变更背景与目的
- 解决模型将用户画像当作冷硬数据库清单（`- key: value`）进行对齐与合规审计的问题。
- 去除暴露数据库内部 key 的格式，改为温暖、自然的陈述点列表。
- 标题由冷硬的 `【核心画像】` 柔化为自然的 `关于{user_name}：`。

### 实际修改文件
1. `gateway.py`（`HostFixMiddleware._inject_context`，约 2394 行与 2511 行）：
   - 提取 `user_facts` 的行改为只取 `val[:150]`，前缀为圆点 `• `，截取条数控制在 30 条内，隐藏数据库 key。
   - `volatile_block` 中的标题由 `【{user_name}的核心画像】:` 改为 `关于{user_name}：`。
2. `server.py`（`_build_channel_context`，约 976 行与 1045 行）：
   - 同步修改画像生成逻辑为点状自然语言描述（`• val`），隐藏 key。
   - `volatile_parts` 中的标题同步调整为 `关于{user_name}：`。

### 验证标准检查
- `grep -n "核心画像" gateway.py server.py` 返回为空。
- 注入后的 Prompt 中不再包含 `- key: value` 机械格式。
- 端到端抓取 `gateway._capture_injected_prompt` 实际生成的 `volatile_block` 快照已核验（见任务报告）。
- `python -m py_compile gateway.py server.py` 编译通过。
- 未新增环境变量（无需修改 `VARIABLES.md`）。
