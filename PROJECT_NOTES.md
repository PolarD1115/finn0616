...
### 日记日切点改为 04:00（2026-09-19）

**原因**：半夜聊天后，按自然日 0:00–0:00 切窗会把 0 点后内容甩出「昨日回溯」，当日日记像被丢掉一大段。

**改动**：
- `heartbeat.py`：新增 `DIARY_DAY_CUTOFF`（默认 `04:00`），日总结 memories / 分层记忆窗口改为 **[昨天切点, 今天切点)**；`DIARY_TIME` 默认同步为 `04:00`；若 `DIARY_TIME` 早于切点启动时告警。
- `VARIABLES.md` / `.env.example`：登记新变量并更新默认说明。

**不影响**：周/月/年总结窗口逻辑；已生成旧日记不回写。
...
### v5.5 — 画像注入上限回退 30 → 60（修画像静默丢弃）（2026-09-13）
**性质**：读取侧上限调整。不改数据库、不改查询语句、不改过滤逻辑。

**问题现象**：核查「用户画像注入上限是几条」时，发现实际上限为 **30 条**；而 `user_facts` 经 `_is_profile_key` 过滤后的真画像已达 **38 条** → 按 key 排序后靠后的 **8 条被静默丢弃**，从不进入 prompt，也无任何日志提示。

**溯源（git 实查，已确认）**：
- v3.9 记录的上限是 `rows[:60]`。
- 提交 `0af63a0`（2026-09-03「思考链修复」，全提交仅 9 行改动）把画像拼装从 `- {key}: {value[:200]}` 配 `rows[:60]` 重写为 `• {value[:150]}` 配 `rows[:30]`。
- 结论：**上限降低与格式重写是同一次有意的改动**，并非"拉错版本"导致的整文件覆盖（提交 stat 显示 gateway.py/server.py 各只动了 9 行）。

**实际修改**（3 处 `[:30]` → `[:60]`；**无新增环境变量**）：

| 文件:行 | 位置 | 说明 |
|------|------|------|
| `gateway.py:2814` | `_inject_context` 画像拼装 | 全渠道（Web/TG/QQ）实际注入点 |
| `gateway.py:4513` | 第 43 阶段 ContextPreview 去重基底 | 该处注释即写明「与 _inject_context 同查询/同过滤/同截断」。**必须同步**，否则预览去重基底只含前 30 条，第 31–60 条实际已注入却未被纳入比对 → 预览判定失真 |
| `server.py:985` | `_build_channel_context` 画像拼装 | MCP 工具层，口径与 gateway 对齐 |

**未改动**（本次只动上限，保留 `0af63a0` 的格式选择）：
- 单条截断仍为 **150 字符**（v3.9 时代的 200 字符未回滚）
- 格式仍为 `• {value}`，不再显示 `key` 名
- 查询的 `neq` 排除系统键、`.order("key")` 稳定排序、`_is_profile_key` 过滤，全部未动
- 前端 `/api/profile` 画像页展示为全量（无条数截断），未改

**验证**：
- `py_compile gateway.py server.py` 通过
- 全仓 `[:30]` 精确复核：gateway.py 仅剩 `url[:30]`（Supabase 连接日志截断），画像路径无残留 ✅
- 用 `ast` 提取 `gateway.py` 里**真实的** `_is_profile_key` + `_SYSTEM_PROFILE_KEYS`（不 import 模块，避免模块级副作用）对真实 39 个 key 过滤：39 → 38，套用 `[:60]` 后 **38 条全部注入、丢弃 0 条** ✅
- 对真实表跑等价 SQL：过滤后 38 条、空值跳过 0 条；注入文本量 **2677 字符 → 3616 字符（+939 字符 ≈ +26%）**

**Supabase 操作声明**：仅只读查询，未修改 schema 或数据
**Pinecone 操作声明**：未涉及
...
**D2 · 画像反思周更**（新模块 `memory_profile_reflect.py`）：
- 挂在 `heartbeat._perform_deep_dreaming` 周日周总结之后；读取真画像（排除 sys_config/llm_*/desire 运行时）+ 近 7 天 active long_term/moment/current。
- 校验顺序：段落完整保留（旧段须为新文子串）→ 长度 ≥ 原长 × 0.9 → 最多新增 5 句；失败拒绝写入。按 key upsert，不删 key；可新增 `profile_<topic>_<hash6>`。
- 门控 `PROFILE_REFLECT_ENABLED`（默认 true）；日志只打 key/长度/计数。

**测试**：新增 `test_memory_diary_bridge_phaseD3.py`（11）+ `test_memory_memo_phaseD1.py`（8）+ `test_memory_profile_reflect_phaseD2.py`（10）共 **29 例全通过**；C1 辅助函数在跑 `_perform_deep_dreaming` 时临时关 `PROFILE_REFLECT_ENABLED`，避免 D2 周日查询干扰「分层门控关 → 零 memory_items 查询」断言。回归 A/C1 等相邻用例通过。`py_compile` 全部改动文件通过。

**新环境变量**（已写入 `VARIABLES.md` §18）：
- `MEMORY_MEMO_ENABLED`（默认 `true`）/ `MEMORY_MEMO_SILENCE_HOURS`（默认 `6`）
- `PROFILE_REFLECT_ENABLED`（默认 `true`）
- `DIARY_MEMORY_BRIDGE_ENABLED`（默认 `true`）

**Supabase 操作声明**：未修改 schema / RLS；D 阶段写入为 service_role INSERT/UPDATE `memory_items` 与 upsert `user_facts`（门控默认开，部署后按需关闭）；本阶段未在真实库执行。
**Pinecone 操作声明**：未涉及。
...
**A1–A3 · 多渠道 memory_events 双写**（`napcat.py` / `heartbeat.py`）：
- A1 QQ：`napcat._handle_merged` 记忆入库后双写（`channel="qq"`、`created_by="napcat"`），**仅主成功路径**——AI 未配置/空回复的固定兜底文案不是真实对话，不进账本（注释已说明理由）；
- A2 TG：heartbeat TG `_handle_merged` 在 memories + Pinecone 写入之后双写（`channel="tg"`、`created_by="heartbeat"`），同样仅主成功路径；
...
## AI 待办系统：阶段 1——基础数据层（2026-09-04）

### 调查到的现状
- Git 状态：工作区已有与本任务无关的未提交修改（`PROJECT_NOTES.md`、`gateway.py`、`server.py`、`prompts/reply_rules.md`、`test_shared_experience_phase6.py`、若干 pyc 及大量未跟踪调研文件），全部原样保留，未覆盖、未回滚、未提交。
- 迁移命名规则：`migrations/` 采用 `YYYYMMDD_NNN_描述.sql`；不存在任何 `20260904_*` 文件，无同名/冲突编号，按任务指定名新建。
- 旧 `reminders` 表不在仓库迁移中定义（`findstr` 全迁移零命中），是早期直接建在 Supabase 的表；代码侧只有两处使用：
  - `server.py` `manage_reminder`（约 1511 行）：add/delete/pause/resume/list，字段 `id`(text 如 R+时间戳)、`time_str`(HH:MM 文本)、`content`、`is_repeat`、`is_paused`、`last_fired`；
  - `heartbeat.py` `async_reminder_worker`（约 1080 行，2153 行注册）：每分钟扫描 `is_paused=false`，`current_hm == time_str` 且当日未触发即推送，LLM 用 `ask_role("background")` 润色，重复项写 `last_fired`，一次性项直接 `delete`。
  → 两者本阶段一律未改动，新系统用独立表 `ai_todos`，与旧表无任何交集。
- `updated_at` 触发器：全部迁移文件中无任何 `TRIGGER` 定义（findstr 零命中），项目没有统一触发器可复用；按约束不为本阶段新建通用触发器，`updated_at` 保留字段由后续应用层显式写入。
- 仓库中没有现成的迁移 SQL 静态检查工具（仅 `tool_loop.py` 有一处注释引用迁移文件名），验证采用任务指定的 grep 检查。

### 新增文件
- `migrations/20260904_002_ai_todos.sql` —— 唯一代码产出。

### ai_todos 的用途
「AI 待办 / AI 想说的话」独立数据表：AI 记录未来要提醒用户的事，或某个时间想主动对用户说的话；到时间由网关后台推送到 Telegram。控制台 / Mini App 后续经网关 API 展示与管理。不是多用户任务管理系统：无子任务、无真实完成判定、无复杂优先级、无完整 RRULE、无多渠道配置。

### kind 与 delivery_mode 的区别
- `kind` 描述「这是一件什么事」：`reminder` = 提醒事项（吃药、上课、出门）；`message` = AI 想在某时间主动说的一段话（月初、生日、纪念日）。
- `delivery_mode` 描述「到时间怎么产生要发的内容」：`static` = 直接发送 `content`，不调用模型；`dynamic` = 结合有限上下文调用 background 模型生成，`generation_prompt` 存表达要求，`fallback_content` 存生成失败时的备用内容（`content` 在 dynamic 模式下同样可作兜底）。

### status 的真实含义
`pending` 等待触发；`sending` 正在发送（防后台重复领取的幂等锁语义）；`completed` 已处理、**不再继续调度**；`cancelled` 已取消。`completed` 只表示调度终结，**不代表系统判断用户真的吃药、上课或完成了现实中的事情**。重复待办发送成功后不能改为 completed，后续阶段会计算下一次 `scheduled_at` 并继续保持 pending。

### repeat_rule 约定
第一版仅允许应用层接受四种格式：`NULL`（不重复）/ `{"type":"daily"}` / `{"type":"weekly","weekdays":[1,3,5]}` / `{"type":"weekdays"}`。数据库不做复杂 CHECK，合法性由应用层校验；本阶段只保存字段，不实现规则计算。

### 索引与 RLS
- 部分索引 `idx_ai_todos_pending_scheduled_at (scheduled_at ASC) WHERE status IN ('pending','sending')`：覆盖调度器热路径 `status IN ('pending','sending') AND scheduled_at <= now()`；
- `idx_ai_todos_status_scheduled_at (status, scheduled_at ASC)`：全状态扫描/列表兜底；
- `idx_ai_todos_kind (kind)`：按类型筛选。
- RLS 沿用 `activity_logs`/`memory_events` 敏感业务表风格：`ENABLE ROW LEVEL SECURITY` + 不创建任何 anon/authenticated 策略（deny-by-default）+ `REVOKE ALL ... FROM anon, authenticated`；读写后续均走网关 service_role，前端只能调网关 API。
- 另有 `COMMENT ON TABLE` 与 `kind/delivery_mode/status/repeat_rule` 四列的 `COMMENT ON`，内容均为普通语义说明，无敏感信息。

### 明确未做的事
- 未执行数据库迁移（未调用 Supabase，无任何 DDL/DML 实际执行），仅创建 SQL 存档文件；
- 未修改旧 `reminders` 表、`manage_reminder`、`async_reminder_worker` 及任何相关代码；
- 未新增环境变量，未修改 `VARIABLES.md`、部署与 Docker 配置；
- 未修改 `server.py` / `gateway.py` / `heartbeat.py` / `console.html` / `miniapp.html` 业务逻辑；
- 未实现 MCP 工具、调度器、动态生成、Telegram 发送、推送记录表、reminders 数据迁移；
- 未删除任何表/记录/字段/约束，迁移内无 DROP/DELETE/TRUNCATE，全部 `IF NOT EXISTS` 幂等可重复执行。

### 验证
- `python -m py_compile gateway.py server.py heartbeat.py` 通过（本阶段未修改这三个文件，编译确认未被误伤）；
- `grep -nE 'DROP TABLE|DELETE FROM|TRUNCATE|DROP COLUMN|DROP CONSTRAINT' migrations/20260904_002_ai_todos.sql` 无任何输出；
- `git status --short` 核对：本阶段仅新增 `migrations/20260904_002_ai_todos.sql` 并追加本日志，`VARIABLES.md` 未修改，已有未提交修改原样保留；
- 未执行任何数据库操作。

### 尚未实现（后续阶段）
- `manage_ai_todo` 类 MCP 工具与网关 API（创建/列表/取消/完成）；
- 后台调度器（领取 `pending AND scheduled_at <= now()` → `sending` → 发送 → 计算 repeat 下一次或 completed）；
- dynamic 模式的上下文读取与 background 模型生成、失败兜底链路；
- Telegram 实际发送与 `last_generated_*` / `last_sent_at` / `last_error_code` 的写入链路；
- 控制台 / Mini App 展示与管理界面。

### 已知风险或待确认点
- `sending` 状态的恢复策略（如进程崩溃后卡在 sending 的超时回收）留待调度器阶段定义，表结构已预留该语义；
- `timezone` 当前仅保存展示用途，触发换算以 `scheduled_at`（timestamptz）为准，是否参与重复规则计算由后续实现阶段决定；
- 迁移文件尚未在 Supabase 执行，部署时需人工审阅后执行，执行前 `ai_todos` 不存在于生产库。

---

## AI 待办系统：阶段 2——MCP 工具与服务端业务层（2026-09-04）

### 只读调查结论（先行于任何修改）
- **MCP 工具结构**（子任务 A）：server.py 共 2951 行，FastMCP v1（`FastMCP("GenericGateway")`，SDK v2 直接 SystemExit）；73 个 `@mcp.tool()` 全部 `async def`，63 个叠加 `mcp_error_handler`（L407-415 装饰器：捕获 Exception 返回 `❌ 工具执行出错: {e}`，不打日志）；Supabase 初始化 L82-101（`SUPABASE_URL`+`SUPABASE_KEY` 齐全才 `create_client`，否则全局 `supabase=None` + print）；标准访问模式=判空 → `asyncio.to_thread(lambda: supabase.table(...))` → `res.data`；`_get_now_bj`（L608）返回 naive+8h，无 zoneinfo/dateutil，无现成"字符串→aware datetime"工具（需自建）；无 logger 框架（print + emoji 是项目日志风格）。
- **旧提醒链路**（子任务 B）：`reminders` 表的全部 8 处访问封闭在 `manage_reminder`（server.py 1511-1543）与 `async_reminder_worker`（heartbeat.py 1080-1149）内，其余文件零引用；`manage_ai_todo`/`_parse_ai_todo_time`/`_normalize_ai_todo_repeat_rule`/`_validate_ai_todo_enum`/`_format_ai_todo_row` 等命名零冲突（server.py 中 "todo" 零出现）；旧表用 `time_str`(HH:MM 文本)+`last_fired`(日期串) 字符串匹配，新表用 timestamptz `scheduled_at`+状态机，语义无交集。
- **Supabase 与测试方式**（子任务 C）：项目测试为 unittest（requirements 无 pytest），`python -m unittest test_xxx -v`；`test_activity_logs.py` 提供 FakeSB/_FakeTable 链式 mock + `patch.object` 注入模式；多个既有测试直接 `import server`（import 安全，FastMCP 装饰器返回原函数，可直接调用）；async 用 `IsolatedAsyncioTestCase`；实测本机 supabase 2.31.0 builder 支持 `in_/order/eq/limit`。
- **工作区**（子任务 D）：开始前记录 `git status`，已有未提交修改（PROJECT_NOTES.md、gateway.py、server.py 的 `_build_channel_context` 区域 976-1120 行、prompts/reply_rules.md、test_shared_experience_phase6.py 等）全部原样保留；heartbeat.py 与 VARIABLES.md 零修改；阶段 1 日志位于 PROJECT_NOTES.md 末尾。

### 实际修改文件
- `server.py`：仅在最后一个工具 `archive_note` 之后、Phase 6 安全收口注释之前插入一个自包含区块（约 369 行）——6 个私有辅助函数 + `manage_ai_todo` 工具。既有未提交修改区域（`_build_channel_context`）未触碰（git diff hunk 可证：前 4 个 hunk 均为任务前已有，第 5 个 hunk 即本区块）。
- `test_ai_todo_tools.py`：新增定向 mock 测试（41 个用例）。
- `PROJECT_NOTES.md`：末尾追加本日志。

### 新增 MCP 工具：manage_ai_todo
- 签名：`manage_ai_todo(action, todo_id="", kind="", delivery_mode="", title="", content="", generation_prompt="", fallback_content="", time_str="", repeat_rule="", status="")`。
- 与任务给定签名的两处有意偏离（均有依据）：
  1. **kind/delivery_mode 默认值改为 `""` 而非 `"reminder"`/`"static"`**：任务正文要求 add 必填这两项；非空默认值会使 update 分支无法区分"未传入"与"显式传入"，导致 update 时把 message/dynamic 默默覆盖回 reminder/static（测试实锤了该问题）。空默认值下 add 时为空会被枚举校验拒绝（符合"必填"语义），update 时空=不修改。
  2. **新增 `status=""` 参数（仅 list 使用）**：任务要求 list 至少支持按 pending/sending/completed/cancelled/all 筛选，一个默认空参数是最小方案（未为此加"大量参数"，也未复用语义无关的 repeat_rule）。
- 私有辅助函数（全部直接服务本工具，无一次性抽象）：
  - `_ai_todo_now()`：UTC aware now（过期校验与 updated_at 统一来源）；
  - `_parse_ai_todo_time(time_str)`：ISO（含 Z/+08:00）与 "2026-09-05 21:00" 空格写法 → aware datetime；无时区按 Asia/Shanghai（`timezone(timedelta(hours=8))`，不用服务器本地时区）；带 +08:00 时 tz 字段记 Asia/Shanghai，其他偏移记 `UTC±HH:MM`；
  - `_normalize_ai_todo_repeat_rule(raw)`：空/None/JSON "null"→NULL；`{"type":"daily"}`/`{"type":"weekdays"}`/`{"type":"weekly","weekdays":[1-7]}` 三种白名单格式（拒绝未知 type、拒绝多余字段、weekly 的 weekdays 去重排序、拒绝布尔与非 1-7 值）；
  - `_describe_ai_todo_repeat`/`_format_ai_todo_row`：模型可读的重复规则描述与列表行格式化；
  - `_validate_ai_todo_enum`：kind/delivery_mode 枚举校验（错误文案与任务要求一致）。
- **action 语义**：
  - `add`：枚举校验 → title 非空（≤200 字）→ 内容长度（content/generation_prompt/fallback_content 各 ≤5000 字）→ static 必须 content 非空、dynamic 必须 generation_prompt 非空 → 时间解析且必须晚于当前（过去时间直接拒绝，不顺延不修正）→ repeat_rule 规范化 → insert（scheduled_at 存 UTC isoformat、status=pending）。dynamic 语义：generation_prompt 存表达要求，fallback_content 为优先备用内容，content 为兼容保留的备用内容，两者皆空时生成失败只记录不编造。
  - `list`：默认 `in_(status, ["pending","sending"])`，单状态 `eq`，`all` 不过滤；`order(scheduled_at asc)` + limit 50 + Python 侧排序兜底；输出为模型可读多行（[提醒事项]/[想对你说]、北京时间、方式、重复、ID），单状态/all 视图附状态行。
  - `update`：只收集传入的非空字段（strip 后空=不修改），逐字段校验；显式写 `updated_at`；受保护字段（id/created_at/last_sent_at/status）不在写入通道中（status 参数在 update 分支完全不使用）；先 select 存在性再 update，不存在的 todo_id 返回"未找到"；`repeat_rule` 传 "null" 可清除重复。
  - `complete`/`cancel`：先查存在性 → 仅 update `{status, updated_at}` 两个字段；不 delete、不动其他列；complete 的返回文案明确"仅表示停止调度，不代表用户现实中已完成"。
  - 未知 action：返回 `❌ 未知操作。支持：add、list、update、complete、cancel`。
- **数据库访问**：完全复用全局 `supabase` 客户端与 `asyncio.to_thread` 模式，无第二客户端、无环境变量读取；表名 `ai_todos` 硬编码在代码内，模型不可指定表名/用户 ID/查询条件；不把 Supabase 响应原样返回。
- **异常处理**：整体由 `@mcp_error_handler` 兜底；数据库操作有针对性 try/except 补充上下文，print + emoji（项目现有日志风格）+ `traceback.format_exc()` 记录完整堆栈，日志只含 action/kind/mode/todo_id 等非敏感字段，不记 content/prompt/密钥；给模型的错误为简洁中文（如"数据库暂不可用或 ai_todos 表尚未创建"），不暴露堆栈与连接信息。

### 测试（test_ai_todo_tools.py，41 用例，纯 mock）
- 覆盖：add 成功/失败 6 场景、时间解析 3 场景（+08:00 转 UTC 保存、无时区按北京时间、非法中文时间）、重复规则 11 场景（四种格式+非法 type/缺 weekdays/越界/布尔/多余字段/非法 JSON/roundtrip）、list 5 场景（默认 pending+sending、all、单状态、非法 status、空列表+升序断言）、complete/cancel 3 场景（只改 status+updated_at、不 delete 记录保留、不存在 ID）、update 7 场景（只更新传入字段、空默认不覆盖、status 误传不被写入、repeat 清除、不存在 ID、无字段、时间非法/超长）、安全 4 场景（数据库未连接、未知 action、全程只碰 ai_todos 表不碰 reminders、输出不含 supabase.co/eyJ/SECRET/Traceback）+ 数据库故障友好文案（注入 `SECRET_TOKEN`+假 URL 的异常，断言不泄漏到输出）。
- FakeSB 记录 `table_names` 与 `delete_calls`，用于"不触旧表""不删数据"断言。

### 验证结果
- `python -m py_compile server.py test_ai_todo_tools.py`：通过。
- `python -m unittest test_ai_todo_tools`：41/41 通过（Ran 41 tests, OK）。pytest 未安装（requirements 无此依赖），按任务规则沿用项目现有 unittest 方式，未安装/未切换框架。
- 旧提醒兼容：`git diff -- heartbeat.py` 为空（零修改）；server.py diff 中含 "reminders" 字样的仅为本区块说明注释，`manage_reminder` 函数本体无任何改动；测试断言全程只访问 `ai_todos` 表。
- 危险操作检查：`findstr /S /N /I "DROP TABLE DELETE FROM TRUNCATE DROP COLUMN DROP CONSTRAINT" server.py test_ai_todo_tools.py` 零命中。
- `git diff -- VARIABLES.md` 无输出（未修改）；未新增任何环境变量；未执行任何真实 Supabase 读写删除、未执行迁移（生产库零变更，测试全 mock）。
- git diff hunk 核对：server.py 前 4 个 hunk 为任务前已有未提交修改（未触碰），仅新增 archive_note 后的区块。

### 尚未实现（后续阶段）
- 后台到时调度（heartbeat.py 新 worker：领取 pending+到期 → sending → 发送 → repeat 推进下一次或 completed）；
- Telegram 实际发送（可复用 `_push_wechat`）与 last_sent_at/last_error_code/last_generated_* 写入链路；
- dynamic 模式到时结合上下文调用 background 模型生成与 fallback 链路；
- gateway.py API、console.html / miniapp.html 展示管理。

### 已知风险或待确认点
- update 无法"清空"content/fallback_content 等文本字段（空字符串=不修改，这是防误覆盖的代价）；改用传新值覆盖即可，无需清空场景。
- update 不做跨字段完整性校验（如把 delivery_mode 改为 static 但未同时给 content，会留下 static 且 content 为空的记录）；到时调度阶段应把"static 但无 content"视为异常记录（记错误码，不崩溃）。
- FastMCP v1 对 11 参数签名的兼容性：与现有工具（7+ 参数）风格一致，风险低；若某客户端对参数数量受限，可按任务预案改为等价 JSON 参数，本阶段未需要。
- `ai_todos` 迁移仍未在 Supabase 执行：生产库无此表前，本工具的 add/list 会返回"数据库暂不可用或 ai_todos 表尚未创建"的友好错误（测试已覆盖该路径），不会崩溃网关。

---

## AI 待办系统：阶段 3——后台调度与 Telegram 发送（2026-09-05）

### 并行只读调查结论（先行于任何修改）
- **后台启动链路**（子任务 A）：`run.py` 拉起 server.py（进程A）+ background.py（进程B）；background.py 调 `asyncio.run(run_background_process())`。`run_background_process`（heartbeat.py:2138 起）用 `asyncio.create_task` 汇集全部 worker 并 `asyncio.wait(FIRST_EXCEPTION)`——**任一 worker 抛异常会取消其余任务并整体退出、由 run.py 重启**，因此新 worker 必须把一切异常消化在循环内。worker 惯用形态：`while True → sleep → try/except + print(emoji)`（参照 `async_reminder_worker` heartbeat.py:1080-1149、`async_unified_autonomy`）。Supabase 客户端复用 `server.supabase` 全局，函数内延迟 `from server import ...` 是本文件惯例（避免循环依赖）。
- **Telegram 推送链路**（子任务 B）：`server._push_wechat(text, title, plain)`（server.py:777）是既有推送函数，但**吞掉一切异常且不检查 HTTP 响应**，调用方无法感知发送成败——若复用它，发送失败的任务会被错误标记 completed，违反本阶段"失败不标记完成"的硬性要求。heartbeat.py:826 闭包 `_send_message`（`requests.post + raise_for_status + 检查 ok 字段`）具备成败感知，但是 `async_telegram_polling` 的内部闭包无法导入。结论：在 heartbeat.py 新增模块级 `_ai_todo_send_telegram`，**复用与 `_send_message` 完全相同的 Bot API 调用方式**（非第二套客户端），纯文本发送（不带 parse_mode，规避正文未配对 markdown 被 TG 400 拒收且被吞的问题）。`telegram_enabled` 开关在 `gateway._tg_enabled()`（gateway.py:1116-1118，读 sys_config 带 5s TTL 缓存，默认 True），heartbeat 内既有 `import gateway as _gw` 延迟导入惯例。
- **模型与上下文链路**（子任务 C）：`ask_role(role, prompt, system_prompt="", temperature=0.7) -> str`（server.py:522）为 async，返回已剥 `<thinking>` 的纯文本，全部端点失败返回 `""`；`background` 角色池自带逐端点故障转移与回退默认模型，无需调用方处理。`_build_channel_context(query, channel_tag, inject_device, source)`（server.py:892）为 async，内部 `_safe` 逐数据源降级不抛异常，返回值可直接作 system_prompt；既有先例：主动问候（heartbeat.py:165-198，上下文放 system_prompt、业务指令放 prompt）与旧提醒文案生成（heartbeat.py:1111-1127）。项目无独立的"去 markdown"输出清洗函数，围栏剥离参照 `_parse_decision_json`（heartbeat.py:30-41）的去围栏正则。
- **阶段 2 工具与数据结构**（子任务 D）：`ai_todos` 的 `scheduled_at` 存 aware UTC isoformat（`+00:00` 后缀字符串），`repeat_rule` 存 jsonb（dict 或 NULL），`updated_at` 无触发器需应用层显式写；阶段 2 辅助函数 `_ai_todo_now()` 与 `_AI_TODO_TZ_BJ`（固定 UTC+8）直接复用。阶段 2 `update` 分支（server.py:3127-3180）确无模式-内容配套校验，`static 且 content=""` 的历史脏记录可能出现——按计划在调度入口兜底而非改阶段 2 代码。测试 fake（`_FakeTable.execute` 只认最后一个 op）无法处理 `update().eq().select()` 尾链，且缺 `lt/lte` 过滤——在阶段 3 测试文件中**继承扩展**（`_SchedTable` 按 PostgREST return=representation 语义重写 execute），存储与 `_FakeSB` 语义完全复用，未造第二套数据格式。
- **环境探测**（实测本机 Python 3.11 + supabase-py 2.31.0）：`zoneinfo` 在 Windows 缺 tzdata **不可用**（`ZoneInfoNotFoundError`），时区沿用阶段 2 的固定 UTC+8 `timezone` 对象（中国无夏令时，固定偏移语义安全）；update builder 链式支持 `eq/lt/lte/in_/select`，`.update().eq().eq().select("*")` 条件更新链可构建，`data` 返回命中行、未命中为空列表（PostgREST `return=representation`，即"未命中=被其他实例领取"的判定依据）。gateway.py 模块级无重副作用（Supabase 惰性初始化），import 安全。
- **工作区状态**：任务前已有未提交修改（PROJECT_NOTES.md、gateway.py、server.py 451 行 diff、prompts/reply_rules.md、test_shared_experience_phase6.py 及大量 untracked 文件）全部原样保留；本阶段零触碰。`.gitignore:48` 存在 `test_*.py` 规则——测试文件本就不入库（阶段 2 的 test_ai_todo_tools.py 同样不出现在 git status），属项目既有约定。

### 实际修改文件
- `heartbeat.py`：**+421 行纯新增、零删除行**（git diff 核对），两处改动：
  1. 在旧提醒巡视器之后、日程小秘书之前插入自包含区块"4.5 AI 待办调度器"：代码常量（sending 超时 15 分钟、瞬时退避 5 分钟、无效数据退避 1 小时、TG 正文 4000 字截断、单轮限量 20 条）+ `_ai_todo_mask_secret`（日志脱敏：bot token / JWT / Supabase 主机名）+ `_ai_todo_send_telegram` + `_ai_todo_compute_next_scheduled`（重复规则下一次时间）+ `_ai_todo_wrap_body`（reminder 前缀/message 原样）+ `_ai_todo_strip_fences`（模型输出去围栏）+ `_ai_todo_dynamic_prompt`（动态生成提示）+ `_ai_todo_build_context`（有限上下文，失败降级空串）+ `_ai_todo_process_row`（单条处理）+ `_ai_todo_tick`（单轮巡检）+ `async_ai_todo_worker`（60 秒循环）；
  2. `run_background_process` 的 tasks 列表新增一行 `asyncio.create_task(async_ai_todo_worker(), name="ai_todo")`。
- `server.py`：**未修改**（git diff 451 行全部为任务前已有未提交修改；阶段 2 完整性问题在调度入口兜底，不动阶段 2 代码）。
- `test_ai_todo_scheduler.py`：新增 44 用例（扩展阶段 2 fake：`_SchedTable` 补 `lt/lte` 过滤与 `update...select` 尾链的 return=representation 语义；`_FakeSB` 的 table_names/delete_calls 语义继承复用）。
- `PROJECT_NOTES.md`：末尾追加本日志。

### 后台调度
- **worker**：`async_ai_todo_worker`，每 60 秒一轮（`await asyncio.sleep(60)`）；注册位置 `run_background_process`（tasks 列表，name="ai_todo"）；worker 与 tick 级均为双层 try/except 兜底 + 带堆栈的脱敏日志，**任何异常不退出 worker**（退出会导致整个后台进程被 run.py 重启）。
- **到期查询**：`status = 'pending' AND scheduled_at <= 当前 UTC`（aware UTC isoformat 字符串交给 PostgREST timestamptz 比较），`order(scheduled_at asc)` + `limit(20)` 防积压阻塞。
- **领取（原子）**：条件更新 `UPDATE ai_todos SET status='sending', last_attempt_at=now, updated_at=now WHERE id=? AND status='pending'`，用 supabase-py 2.31 链式 `.update(payload).eq("id",tid).eq("status","pending").select("*")` 实现（探测已确认该链可构建；`select` 触发 return=representation）；**返回 data 非空=领取成功并拿到最新行，为空=已被其他实例领取，直接跳过不发送**。未验证假设：真实 Supabase 上的该行为依据 postgrest-py 2.31 公开 API 语义与 mock 测试（本阶段禁止连生产库）。
- **sending 超时恢复**：常量 `_AI_TODO_SENDING_TIMEOUT = 15 分钟`（代码常量并注释说明；不新增环境变量）；每轮先查 `status='sending' AND last_attempt_at < now-15min`（领取时必写 last_attempt_at，故超时即可判定为进程崩溃遗留），条件更新恢复 `pending` + `last_error_code=sending_timeout_recovered`；回收后若 scheduled_at 已到期，本轮即重新投递（崩溃恢复快速重发）。绝不回收刚进入 sending 的任务（条件即 15 分钟阈值）。
- **失败退避**：数据无效（invalid_static_content / invalid_dynamic_prompt / telegram_not_configured）退避 1 小时；瞬时失败（telegram_send_failed / dynamic_generation_failed / process_error）退避 5 分钟；实现方式为恢复 pending 并顺延 scheduled_at——任务不丢、不伪装完成、不每分钟刷日志。所有写回均带条件（领取前 eq pending / 领取后 eq sending），用户在后台处理期间 complete/cancel 的决定不会被覆盖。

### 发送逻辑
- **流程**：完整性校验（入口）→ 原子领取 → 准备文本 → 发送 Telegram → 条件更新推进状态。
- **static**：直接用 `content`；content 为空则不调模型不调 Telegram，退避 1 小时 + `invalid_static_content`。
- **dynamic**：`_build_channel_context(query=标题+generation_prompt 截断 150 字, channel_tag="TG_MSG", source="background_ai_todo")` 作 system_prompt（内部已有全量降级，外层仅兜 import 级意外）；业务提示由 `_ai_todo_dynamic_prompt` 组装（当前北京时间 + 标题 + generation_prompt 原文 + 写死的输出约束：只输出最终正文、纯文本、不要 JSON/markdown/机制词）；调 `ask_role("background", prompt, system_prompt=ctx, temperature=0.85)`；结果 strip + 去代码围栏，为空视为失败。生成成功且非备用内容时写入 `last_generated_content` / `last_generated_at`。上下文构建失败降级为无上下文继续生成，不视为投递失败。
- **fallback 顺序**：生成失败按 `fallback_content → content` 取备用内容发送；两者皆空则不发送、不编造、恢复 pending + `dynamic_generation_failed`。使用备用内容投递成功时：`last_generated_content` **保持原值不伪装**，`last_error_code` 保留 `dynamic_generation_failed`（记录本次实际用了备用内容）。
- **Telegram**：发送函数 `_ai_todo_send_telegram(text)`（heartbeat 模块级，复用 `_send_message` 同款 Bot API 调用：`requests.post` + `raise_for_status` + 检查 `ok`，纯文本、timeout 15s）；未配置（TG_BOT_TOKEN/TG_CHAT_ID 缺失）抛专用异常 → 退避 1 小时 + `telegram_not_configured`；网络/HTTP 失败 → 退避 5 分钟 + `telegram_send_failed`；正文超 4000 字符截断（Bot API 上限 4096，阶段 2 内容上限 5000，超长兜底）；`telegram_enabled=false` 时本轮不领取任何任务（任务保持 pending 不标记已发送）。**不复用 `_push_wechat` 的原因**：它吞异常且不检查响应，无法感知失败，会把失败任务错误标记 completed。
- **文案**：reminder → `⏰ 小提醒\n\n{content}`；message → 原文直发（AI 主动说的话不加强制标题）。
- **日志**：print + emoji 项目风格；异常消息与堆栈落日志前必过 `_ai_todo_mask_secret`（bot token → `bot***`、JWT → `eyJ***`、Supabase 主机名 → `***`）；不打印完整待办正文、动态提示词、Token、密钥。

### 重复规则（_ai_todo_compute_next_scheduled）
- 一次性（repeat_rule 为 NULL）：发送成功 → `status=completed` + `last_sent_at` + `updated_at` + `last_error_code=''`。
- daily：以本次计划时间为基准 +1 天。
- weekly：从计划时间下一天起在北京时间找下一个允许星期（weekdays 1=周一…7=周日，与阶段 2 保存格式一致）。
- weekdays：仅周一至周五，跳过周末。
- 时区：统一 Asia/Shanghai（固定 UTC+8；**zoneinfo 在本机 Windows 缺 tzdata 不可用，实测确认**，且中国无夏令时，固定偏移语义安全）；重复任务推进后保持 `pending` + `last_error_code=''`。
- 推进失败（非法规则等计算异常）：保持 pending、退避 5 分钟后重发 + `repeat_advance_failed`，**不改 completed 掩盖计算错误**。

### 数据库操作
- 是否执行真实查询/写入/更新/删除：**全部没有**（本阶段禁止连接生产库；所有 DB 交互经 `asyncio.to_thread` + `server.supabase`，测试注入内存 fake）。
- 是否执行迁移：**没有**（`migrations/20260904_002_ai_todos.sql` 保持未执行状态，文件未改动）。
- mock 方式：`test_ai_todo_scheduler.py` 继承阶段 2 `_FakeSB` 的存储语义，`_SchedTable` 扩展 `lt/lte` 与条件更新尾链；`patch.object(server, "supabase", fake)` 注入；Telegram/模型/上下文均为进程内 fake。

### 测试结果
- `python -m py_compile heartbeat.py server.py test_ai_todo_scheduler.py`：**通过**。
- 阶段 3 定向测试 `python -m unittest test_ai_todo_scheduler`：**44/44 通过**（到期扫描 7、原子领取 2、static 3、dynamic 5、fallback 3、Telegram 失败 4、一次性 1、重复推进 4 + 纯函数 6、文案 2、旧系统兼容 3、异常隔离 2、脱敏 2）。
- 阶段 2 回归 `python -m unittest test_ai_todo_tools`：**41/41 通过**。
- 旧 reminders 检查：`git diff -- heartbeat.py` 为 **+421/-0 纯新增**；AST 解析确认 `async_reminder_worker` 函数体（heartbeat.py:1080-1149）未被触碰且不含 `ai_todos`；`server.manage_reminder` 定义原样存在；测试断言全程只访问 `ai_todos` 表、`delete_calls` 恒为空。
- 危险操作检查：正则扫描 `DROP TABLE|DELETE FROM|TRUNCATE|DROP COLUMN|DROP CONSTRAINT` 在三个文件中仅命中 heartbeat.py:216 的一处**任务前已有注释文字**（旧代码，非本阶段引入），无实际危险语句。
- Git diff 检查：`git diff -- VARIABLES.md` 为空；`server.py` diff（451 行）全部为任务前已有未提交修改，本阶段未新增；工作区其他未提交修改与 untracked 文件原样保留。

### 文档与环境变量
- `PROJECT_NOTES.md`：追加本日志。
- `VARIABLES.md`：未修改；**未新增任何环境变量**（仅复用既有 TG_BOT_TOKEN / TG_CHAT_ID；sending 超时 15 分钟、退避 5 分钟/1 小时均为代码常量）。

### 尚未实现（后续阶段）
- gateway API（HTTP 管理接口）；console.html / miniapp.html 展示与管理界面；发送历史完整记录；多用户；用户完成情况判断；QQ/邮件等其他推送渠道；reminders 迁移。

### 已知风险或待确认点
- `.update().eq().select()` 的 return=representation 行为依据 postgrest-py 2.31 公开语义与 mock 验证，**未在真实 Supabase 上确认**（本阶段禁止连生产库）；迁移执行后建议先在控制台手工建一条待办观察一轮投递日志。
- 部署依赖链：ai_todos 表未创建前，worker 每轮查询会报错但被双层兜底捕获（每 60 秒一条错误日志，不崩溃不重启）；生产 `SUPABASE_KEY` 若为 anon key，迁移的 RLS（revoke anon/authenticated）会拒绝读写，需确认实际 key 类型或改用 service 客户端。
- 失败退避会顺延 scheduled_at（pending 状态下 5 分钟/1 小时后重试），该字段因此具备"计划时间+重试时间"双重语义；gateway/console 展示时需知悉。
- telegram_enabled 门控复用 gateway._tg_enabled（5s TTL 缓存），读取失败按开启处理（有日志），极端情况下可能多投递一轮。
---

## AI 待办系统：阶段 4——网关 API 与控制台 / Mini App UI（2026-09-05）

### 并行只读调查结果

- gateway.py：原生 ASGI（无框架），`HostFixMiddleware.__call__` 顺序 if 分发；全局 `/api/*`（非 OPTIONS）统一走 `_check_api_secret`（Authorization Bearer 或 X-Api-Key 任一），失败 401；OPTIONS 走 `_send_cors_preflight`（204）；统一 JSON 响应 `_send_json_resp`；路径参数用前缀切片（memories/profile/letters 先例）；Supabase 用 gateway 自己的惰性单例 `_get_supabase()`（未配置返回 None）；DB 调用包 `asyncio.to_thread`；日志走 `_log`（时间戳 + 内存缓冲）。发现 CORS 缺口：`_send_json_resp` / `_send_cors_preflight` 的 allow-methods 缺 PATCH。
- console.html：`data-p` 导航 + `.page` 显隐路由（`showPage` + loaders map）；已有 `api()`（X-Api-Key 头，401 返回 null、其他非 2xx 抛 Error）、`esc()`、`toast()`、modalRoot 弹窗体系、`fmtCN8()`、`refreshIcons()`、`.card/.item/.tag/.stat/.tabs/.pager/.empty` 全套样式；原生 `confirm()` 是项目现有确认惯例。
- miniapp.html：与 console 同构，但 CSS 把侧栏改造成固定底部 Tab 栏，modal 自动变底部弹层，按钮 min-height 42px；`api()/esc()/fmtCN8()/toast()` 与 console 逐字符一致；两文件此前均无任何 ai-todo 代码（已搜索确认）。
- 阶段 2/3 服务端：`server.py::manage_ai_todo`（add/list/update/complete/cancel）；可复用纯校验函数 `_parse_ai_todo_time`（无时区按 Asia/Shanghai）、`_normalize_ai_todo_repeat_rule`、`_validate_ai_todo_enum`、`_ai_todo_now`、`_describe_ai_todo_repeat` 及常量 `_AI_TODO_TITLE_MAX=200`、`_AI_TODO_TEXT_MAX=5000`；`heartbeat.py::async_ai_todo_worker` 用 `.update(...).eq("id").eq("status")` 条件更新原子领取，status 只有 pending/sending/completed/cancelled（无 paused）；migrations/20260904_002_ai_todos.sql 字段含 id/kind/delivery_mode/title/content/generation_prompt/fallback_content/scheduled_at/timezone/repeat_rule/status/last_*/created_at/updated_at。
- 工作区状态：gateway.py / server.py / heartbeat.py / PROJECT_NOTES.md 在任务前已有未提交修改（git status M），本阶段所有编辑均为追加式，未覆盖已有修改。

### 新增 API（全部在 gateway.py）

- `GET /api/ai-todos`：分页列表；参数 status（默认 pending，支持 sending/completed/cancelled/all）、date_from/date_to（YYYY-MM-DD，按北京时间解释转 UTC 比较）、page（≥1）、size（1-200，默认 50）；按 scheduled_at 升序；`select(*, count="exact")` 取总数；返回 `{"items":[...],"page":n,"size":n,"total":n}`。
- `POST /api/ai-todos`：创建；字段校验完全复用阶段 2 函数（枚举、title ≤200、三段文本 ≤5000、static 必填 content、dynamic 必填 generation_prompt、无时区按北京时间、过去时间拒绝、repeat_rule 只支持 null/daily/weekly/weekdays）；payload 与 `manage_ai_todo add` 一致（status 固定 pending，scheduled_at 存 UTC isoformat）；返回 `{"ok":true,"todo":{...}}`。
- `GET /api/ai-todos/{id}`：单条详情。
- `PATCH /api/ai-todos/{id}`：局部编辑；只接受白名单字段 kind/delivery_mode/title/content/generation_prompt/fallback_content/time_str/repeat_rule；未提供的字段保持原值；传入 id/status/created_at/updated_at/last_*/scheduled_at/timezone 一律 400 拒绝；time_str 重新走解析+未来校验并同步写 scheduled_at+timezone；repeat_rule 传 JSON null 表示改为不重复；改动类型/模式/内容之一时做合并结果态配套校验（static 有 content、dynamic 有 prompt），比 MCP update 略严（防 UI 改出必然失败状态），未放宽任何规则。
- `POST /api/ai-todos/{id}/complete`、`POST /api/ai-todos/{id}/cancel`：软更新 `status=completed/cancelled` + `updated_at`，先查存在（404）、sending 状态 409，再用 `.update(...).eq("id").neq("status","sending")` 条件更新避开与后台 worker 原子领取竞态；**不提供 DELETE 接口，全链路无 `.delete()` 调用**。
- 鉴权：全局 `/api/*` 拦截已覆盖新路由，handler 内又显式调用 `_check_api_secret` 双保险；失败返回与现有端点一致的 401（密钥未配置 503）。
- 错误处理：400 参数错误 / 401 鉴权 / 404 待办或子路径不存在 / 405 方法不允许 / 409 sending 冲突 / 500 数据库错误 / 503 数据库未配置；错误响应为简洁中文 `{"error":"..."}`；日志只记端点、动作、todo_id 和异常类型名，不记 content/prompt/密钥/完整异常消息。
- 极小共享修复：`_send_json_resp` 与 `_send_cors_preflight` 的 allow-methods 补上 PATCH（此前缺失，file:// 跨域场景 PATCH 预检会被浏览器拒绝）。

### 前端不直连 Supabase 的保证

- console.html / miniapp.html 全部通过既有 `api()`（X-Api-Key 头）请求 gateway API，无任何 Supabase JS 客户端调用、无 Supabase Key / API_SECRET 写入 HTML（secret 仍走顶栏输入框 + localStorage，与现状一致）。
- 所有用户内容渲染（title/content/generation_prompt/fallback_content/last_error_code/时间）均经 `esc()` 转义后拼入模板；onclick 内联的 id 经 `todoSafeId()/aiTodoSafeId()` 白名单过滤（脚本自动检查：无裸 `+t.*` 拼接，esc 调用 16/13 处）。

### 控制台（console.html）

- 新增导航项与页面 `#p-todos`（AI 待办 ✅），副标题提示"后台会在指定时间提醒你，或者替你说一段话"。
- 顶部统计三卡（待发送 / 今天 / 已发送），由前端按当前列表计算（北京时间今天）。
- 筛选 chips：全部 / 待发送 / 提醒事项 / 想对你说 / 已发送 / 已取消（前端过滤，无 paused 项）。
- 卡片展示：类型 tag（提醒事项=蓝、想对你说=橙）、发送模式、重复规则、时间（fmtCN8 北京时间）、标题+状态 tag、正文或表达要求（dynamic 截断 200 字）、上次异常、上次发送时间。
- 操作：编辑 / 完成 / 取消；completed 与 cancelled 只保留编辑（不再显示完成/取消按钮）；取消走原生 confirm 确认后调软取消 API，不调 DELETE。
- 新建 / 编辑共用 modal：类型、发送方式（切换显示固定内容 / 表达要求+备用内容两组字段）、标题、datetime-local 时间（北京时间语义，无时区提交由服务端按 Asia/Shanghai 解析）、重复（不重复/每天/每周带周一~周日多选/工作日，weekly 至少一天）；提交前基础校验，服务端校验保留。

### Mini App（miniapp.html）

- 新增底部 Tab 与页面 `#p-aitodos`（AI TODO LIST），移动端卡片布局：类型颜色 tag（重要提醒=蓝 / 想对你说=橙）、清晰时间（MM/DD HH:mm）、大字标题、正文或"到时结合上下文生成"、表达要求截断 160 字、状态脚注、底部大面积 编辑/完成/取消 按钮（复用移动端 button min-height 42px）。
- 同样的六项筛选 chips、新建/编辑底部弹层表单、软完成/软取消（confirm 确认）、成功后重新加载。
- 未引入统计卡、表格或新框架。

### static / dynamic 在 UI 的语义

- static = "固定内容"：卡片显示 content 正文，表单必填"固定发送内容"。
- dynamic = "到时结合上下文生成"：卡片显示"表达要求"（折叠截断），表单必填"表达要求"、可选"备用内容"；生成失败时 worker 按 fallback_content → content 兜底（阶段 3 逻辑，未改动）。

### 时间与重复规则传递

- 前端 datetime-local 提交无时区字符串（如 2026-09-05T21:00），API 复用 `_parse_ai_todo_time` 按北京时间解释、拒绝过去时间、scheduled_at 统一存 UTC isoformat；显示统一走 fmtCN8（UTC+8）。
- repeat_rule 前端组装 `{type:"daily"| "weekdays"| "weekly", weekdays:[...]}`、不重复传 null；服务端复用 `_normalize_ai_todo_repeat_rule` 校验规范化（weekly 去重排序）。

### 数据库操作边界

- 本阶段未执行任何真实 Supabase 查询、写入、更新、删除，未执行迁移；全部测试基于内存 fake 客户端。
- 代码层面只有软更新（update payload 仅含 status/updated_at 或白名单编辑字段 + updated_at），无 DELETE、无 .delete()。

### server.py / heartbeat.py / 旧 reminders

- server.py、heartbeat.py：**未修改**（API 复用其纯校验函数，经函数内 `import server` 惰性导入，与项目现有先例一致）。
- 旧 `manage_reminder` / `async_reminder_worker` / reminders 表逻辑：未触碰。

### 环境变量与依赖

- 未新增环境变量；VARIABLES.md 未修改；未新增任何外部依赖。

### 测试结果

- `python -m py_compile gateway.py server.py heartbeat.py test_ai_todo_api.py`：通过。
- `python -m unittest test_ai_todo_api`：**51/51 通过**（真实走 HostFixMiddleware ASGI 分发 + 全局鉴权拦截 + OPTIONS 预检；mock 只替换 gateway._supabase_client）。
  - 覆盖：鉴权（无头/错 key/X-Api-Key/Bearer/API_SECRET 未配置 503）、GET（默认 pending、status=all、排序、非法 status/page/size/size>200/date 400、date 边界 UTC 换算、分页 range）、POST（static/dynamic 成功、缺 title、过去时间、非法 repeat_rule、static 缺 content、dynamic 缺 prompt、非法 kind、非法 JSON）、PATCH（只改白名单、未传保持原值、status 与受保护字段 400、time 重算、repeat null 清空、weekly 去重、404、dynamic 缺 prompt 400、sending 409、非法 id 400）、详情（200/404）、软完成/软取消（payload 仅 status+updated_at、404、sending 409、错误方法 405）、安全（DB 异常 500 不泄露假密钥与主机名、Supabase 未配置 503、预检含 PATCH、响应无 Token/Key 样式字符串）。
  - 每个测试 tearDown 断言 `delete_calls == 0` 且只触碰 `ai_todos` 表。
- 阶段 2 回归 `test_ai_todo_tools`：41/41 通过。
- 阶段 3 回归 `test_ai_todo_scheduler`：44/44 通过。
- 前端：提取两个 HTML 内嵌 JS 用 `node --check` 校验语法通过；自动扫描确认无未转义动态拼接。
- 危险操作检查：正则扫描 `DROP TABLE|DELETE FROM|TRUNCATE|DROP COLUMN|DROP CONSTRAINT` 于 gateway.py/console.html/miniapp.html/test_ai_todo_api.py 无本阶段新增命中；新增代码无 `.delete()`。
- Git 检查：`git diff -- server.py heartbeat.py` 仅任务前已有修改；`git diff -- VARIABLES.md` 为空；未执行迁移、未覆盖已有未提交修改。

### 未实现 / 未验证

- 真实 Supabase 上 `select(*, count="exact")`、`.neq()` 条件更新、`update(...).select()` return=representation 行为未在线验证（本阶段禁止连生产库）。
- 迁移 20260904_002_ai_todos.sql 仍未执行：表不存在时 API 返回 500（日志每请求一条），前端 toast 展示；执行迁移后即可用。
- UI 层未做真实浏览器点击测试（无浏览器自动化环境下的手工验证）；GET date_from/date_to 筛选在 UI 上暂未暴露入口（API 已支持）。
- status=pending 默认筛选与 MCP 工具默认（pending+sending）不同：API 按阶段规范固定默认 pending，UI 用 status=all 前端过滤，不受影响。

### 已知风险

- API 与后台 worker 属不同进程（run.py 双进程），complete/cancel 用 neq sending 条件更新 + 409 兜底规避竞态，但极端时序下（fetch 后 worker 恰好领取）API 返回 409 需用户重试，属预期行为。
- PATCH 在行处于 sending 时拒绝编辑（409），worker 发送完成通常在数秒内，窗口很短。
- 前端列表一次拉取 status=all&size=200，超过 200 条时列表不完整（单用户场景可忽略）。

---

## AI 待办系统：阶段 5——生产启用与真实联调（2026-09-06）

### 迁移前 SQL 安全检查

- 对 `migrations/20260904_002_ai_todos.sql` 全文复查：
  - 无 `DROP`、无 `DELETE`、无 `TRUNCATE`（正则逐行核对）；
  - 仅含 `CREATE TABLE IF NOT EXISTS public.ai_todos`、`CREATE INDEX IF NOT EXISTS ×3`、
    `ALTER TABLE public.ai_todos ENABLE ROW LEVEL SECURITY`、
    `REVOKE ALL ON public.ai_todos FROM anon, authenticated`、`COMMENT ON ×5`；
  - 不修改任何旧表，不含真实凭据，不覆盖已有表（IF NOT EXISTS）。
- 执行前经 information_schema 确认 `public.ai_todos` 不存在。
- 检查结论：安全，允许执行。

### 迁移执行与只读验证

- 已通过 Supabase 迁移通道执行 `20260904_002_ai_todos`（入册版本号 20260906040136）。
- 执行后只读验证：
  - 表存在，18 个字段与阶段 1 设计逐一一致（kind/delivery_mode CHECK、timestamptz、jsonb、默认值）；
  - 索引 3 个齐全：`idx_ai_todos_pending_scheduled_at`（部分索引 pending/sending）、
    `idx_ai_todos_status_scheduled_at`、`idx_ai_todos_kind`；
  - RLS 已开启且策略数为 0（deny-by-default）；
  - 表权限仅 `postgres` 与 `service_role` 持有全部权限；`anon`/`authenticated` 无任何权限（REVOKE 生效）。

### 真实权限验证与根因复现（本阶段最重要的发现）

- 阶段 2/3/4 代码整条 ai_todos 链路（MCP `manage_ai_todo`、gateway `/api/ai-todos`、
  heartbeat worker）全部使用 `server.supabase`（`SUPABASE_KEY` = anon 角色）客户端；
  而迁移按设计对该表 REVOKE anon 全部权限（与 activity_logs 等敏感表同风格，
  读写应走 `supabase_service`）。阶段 2-4 测试全为 mock，未能暴露此冲突。
- 真实 PostgREST 行为级复现（用 anon key 直连 REST）：
  - `GET /rest/v1/ai_todos` → HTTP 401，`42501 permission denied for table ai_todos`；
  - `POST /rest/v1/ai_todos` → 同样 42501；
  - 对照组：同一 anon key 读 `user_facts` → 200（key 有效，拒绝正是 ai_todos 权限设计所致）。
- 结论：若不改代码，迁移后整条链路必然权限失败（worker 每轮 SELECT 抛错但不退出、
  MCP/API 全部报"数据库暂不可用"）。

### 最小修复（每处 diff 对应真实联调失败）

- `server.py`：`_ai_todo_fetch_one` 与 `manage_ai_todo`（守卫/insert/list/update/complete/cancel）
  全部由 `supabase` 改为 `supabase_service`；fetch_one docstring 注明权限设计。
- `gateway.py`：`_handle_ai_todo…` 分发处由 `_get_supabase()`（anon）改为
  `import server; sb = server.supabase_service`，None 时仍返回 503 中文降级。
- `heartbeat.py`：`async_ai_todo_worker` / `_ai_todo_tick` / `_ai_todo_process_row` 三处
  `from server import supabase` → `supabase_service`（含领取/回收/到期查询/条件更新）。
- 未新增环境变量、未新增依赖、未改动旧 reminders 任何代码。
- 测试适配：三个测试文件的 mock 目标同步改为 `server.supabase_service`
  （test_ai_todo_tools 3 处、test_ai_todo_scheduler 2 处、test_ai_todo_api 4 处含 import server）。

### 真实 API 验证（本地真实网关进程，ASGI 同生产组装）

- 本地以 `uvicorn + HostFixMiddleware(mcp sse_app)` 启动真实网关代码（仅 HTTP，
  不启动 TG 轮询/后台任务，不影响生产）。
- 模式 A（无 SUPABASE_SERVICE_KEY）：health/console/miniapp 均 200（页面含"AI 待办"
  与"AI TODO"入口）；无密钥与错误密钥均 401；ai-todos 全部端点 503 中文降级；OPTIONS 204。
  **8/8 通过**。
- 模式 B（service 客户端用 anon key 顶替 → 通过 503 门后撞真实 42501）：
  鉴权 401、非法 status/page 400、static 缺 content/dynamic 缺 prompt/过去时间/时间格式/
  非法 repeat/非法 kind 均 400 中文；列表/创建/详情/PATCH 的真实权限失败均映射 500
  简洁中文且不泄露主机名与错误细节；非法 ID 400、非法子路径 404、DELETE 方法 405
  （确认无删除接口）。**17/17 通过**。
- 200 成功路径（GET 返回 {items,page,size,total}、POST 返回行、PATCH/complete/cancel
  生效）无法在本地验证：本地无 service key。该行为由 51/51 mock 测试覆盖，且 DB 侧
  等价操作（同角色、同条件更新形状）已在真实库验证（见下）。

### DB 层契约验证（service 权限角色 = 修复后代码所用角色）

- static 一次性：INSERT 到期行 → worker 同形到期查询命中 → 条件领取
  （pending→sending，WHERE status='pending'）命中返回 → 完成推进
  （sending→completed + last_sent_at，WHERE status='sending'）→ 全部符合预期。
  测试行 ID 前缀 `54f3ae7a`。
- 幂等领取：对非 pending 行重复执行领取 UPDATE 返回 0 行（防重复投递机制在真实
  Postgres 上生效）。
- 重复任务 daily：INSERT（repeat_rule={"type":"daily"}）→ 领取 → 推进后 status 仍为
  `pending`、scheduled_at 精确 +1 天（墙钟时分秒保留，与
  `_ai_todo_compute_next_scheduled` 的固定 UTC+8 语义一致）、last_sent_at 已写、
  repeat_rule jsonb 完好。测试行 ID 前缀 `890fcd96`。weekly/weekdays 计算逻辑由
  阶段 3 mock 测试覆盖，未等真实一天/一周。
- dynamic：INSERT → 领取 → 完成态写入 `last_generated_content`（本地真实生成的正文）、
  `last_generated_at`、`last_sent_at` 全部落库。测试行 ID 前缀 `ea932014`。

### dynamic 生成腿真实验证（本地真实模型调用）

- 用与生产同角色（anon）读取 `user_facts.llm_models` 注册表成功（5 个启用模型，
  default 为后台 gemini）。
- `ask_role("background", …)`（heartbeat 真实 prompt 组装 + temperature 0.85）真实调用
  上游 HTTP 200，生成纯正文一句（无围栏、无"待办/调度/系统提示/工具"机制词），
  `_ai_todo_wrap_body("message", …)` 不添加前缀——与 worker 发送路径一致。
- fallback 验证：本地首跑因缺 `openai` 包全部端点失败 → `ask_role` 返回空 →
  走 fallback_content 兜底路径（无异常上抛、无编造）；fallback 的 DB 写入侧由
  44/44 mock 测试覆盖。
- 顺带发现并修复本地环境问题：`openai` 在 requirements.txt 中早已声明，但本地
  解释器未安装；已按项目自身依赖清单补装（`pip install openai`），**不属于新增依赖**，
  requirements.txt 未改动。

### Telegram 真实投递验证

- **未验证**。原因：
  1) 生产环境（Zeabur）运行的是最后一次 commit（`122b9cc`），不含阶段 1-4 的
     ai_todos 代码——阶段 1-4 改动至今未提交；
  2) 工作区混有其他会话的未提交改动（server.py 的 `_build_channel_context` 提示词
     分层、gateway.py 的 refusal/thinking 相关 hunk、heartbeat.py 的
     `_perform_deep_dreaming` 等），整体提交并推送会把无关半成品带上生产，
     属于需要昕决策的提交策略问题，本阶段未擅自 commit/push；
  3) 本地无 TG 凭据（`user_facts` 中无 `sys_config` 行——即 TG 凭据在生产是
     Zeabur 环境变量而非 DB 热同步，本地拿不到，也不能打印）。
- 已确认到什么程度：发送函数 `_ai_todo_send_telegram` 的调用形状与生产 TG 轮询
  `_send_message` 同款（阶段 3 已核）；模型生成腿与 DB 状态机均已真实验证；
  部署后 worker 的 60s 巡检、领取、发送、推进仅剩"运行中的进程 + 真实 TG 凭据"
  这一环待验证。

### 回归测试与编译

- `python -m py_compile gateway.py server.py heartbeat.py`（另含三个测试文件）→ 通过。
- `test_ai_todo_tools`：**41/41 通过**。
- `test_ai_todo_scheduler`：**44/44 通过**。
- `test_ai_todo_api`：**51/51 通过**（mock 目标已随修复同步）。

### 测试数据处理

- 联调测试行共 3 条（前缀 `54f3ae7a` / `890fcd96` / `ea932014`），全部已软取消
  （status=cancelled），表中无 pending/sending 残留，不会触发未来调度。
- **全程未执行任何 DELETE / DROP / TRUNCATE，未删除任何数据、表或代码。**

### 边界确认

- 旧 `reminders` 表与其调度器（`async_reminder_worker` / `manage_reminder`）零改动。
- 未新增环境变量（`VARIABLES.md` 无改动，`git diff -- VARIABLES.md` 为空）。
- 未新增项目依赖（requirements.txt 无改动；本地补装的 openai 为项目已声明依赖）。
- 未修改 `.gitignore`，未删除测试文件，未回滚旧阶段代码。
- 本阶段未创建 commit（原因见 Telegram 一节；如需部署验证，建议将 ai_todos 相关
  hunk 与其他会话改动分离后以 `chore(todo): verify production delivery` 提交）。

### 未验证内容汇总

1. 生产 Zeabur 部署后的 worker 真实投递（static/dynamic → Telegram 实收）。
2. 生产网关 `/api/ai-todos` 的 200 成功路径（需生产 API_SECRET，本地不可得；
   行为由 mock 覆盖）。
3. weekly/weekdays 重复规则的真实跨天推进（mock 已覆盖计算逻辑）。
4. console/miniapp 的真实浏览器点击流（页面 HTTP 200 且含 AI 待办入口已确认）。
5. `sending` 超 15 分钟回收（逻辑 mock 覆盖；真实进程崩溃场景未模拟）。

### 已知风险

- 生产部署时机由昕决定：工作区混有多会话改动，整体提交会带入无关半成品。
- 本地验证中 service 客户端曾以 anon key 顶替（仅本地进程、仅用于触发真实权限
  失败路径），生产无需任何配置变更。
- worker 上线后若 ai_todos 中存在到期任务会立即真实投递到 Telegram——当前表中
  3 条均为 cancelled，部署后不会误发。
