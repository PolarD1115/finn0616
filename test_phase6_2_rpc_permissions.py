...
def _search_memory_source():
    """用 ast 完整提取 server.py 中 search_memory 的源码片段。

    修复（2026-09-13）：旧实现用固定窗口 src[idx:idx+3000] 截取函数体，
    B2 在函数开头新增 memory_items 混合召回后，旧关键词查询段被推出窗口，
    导致静态断言失配（NULL 标签安全逻辑本身仍在）。改用 AST 定位完整函数，
    断言目标不变。"""
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.AsyncFunctionDef, ast.FunctionDef)) \
                and node.name == "search_memory":
            return "\n".join(src.splitlines()[node.lineno - 1:node.end_lineno])
    raise AssertionError("server.py 中应存在 search_memory 函数")
...
class Test01SearchMemoryNullSafe(unittest.TestCase):
    ...
    def test_uses_or_not_neq(self):
        ...
        func_body = _search_memory_source()
